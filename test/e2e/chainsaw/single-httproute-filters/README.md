# Test: `single-httproute-filters`

Verifies that existing HTTPRoute filters (RequestHeaderModifier, ResponseHeaderModifier, RequestMirror, URLRewrite) are preserved on both the original rule and the newly injected header-match rule when header-based routing is active.
...
Create the HTTPRoute with 4 filters and the Rollout. Uses pause:{duration:30s} for auto-promotion.
...
Plugin initializes the HTTPRoute with canary weight 0; the 4 original filters are present.
...
Both the original rule and the injected header-match rule must carry all 4 filters.
...
Plugin removes the header-match rule; original rule still has its 4 filters with canary weight 0.
