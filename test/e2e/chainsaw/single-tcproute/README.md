# Test: `single-tcproute`

Verifies basic weight-based canary traffic splitting with a single TCPRoute. The plugin must set canary weight to 30 when a canary is active and reset it to 0 once the rollout completes.
...
Create the TCPRoute and Rollout. The rollout uses pause:{duration:30s} so it self-promotes.
...
Plugin initializes the TCPRoute with canary weight 0.
