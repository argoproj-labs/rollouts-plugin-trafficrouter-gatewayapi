# Test: `single-header-based-grpcroute`

Verifies header-based routing with a single GRPCRoute. The plugin must inject a header-match rule (X-Test: test) during a canary rollout and remove it once the rollout completes.
...
Plugin initializes the GRPCRoute with 1 rule and canary weight 0.
