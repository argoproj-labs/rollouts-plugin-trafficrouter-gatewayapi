...
// TestHeaderNilWeightHTTPRoute verifies that header route weights remain nil
// (100% traffic to canary for matching requests) while the main route's canary
// weight is updated according to setWeight steps.
func TestHeaderNilWeightHTTPRoute(t *testing.T) {
	feature := features.New("Header nil weight HTTPRoute feature").Setup(
		setupEnvironment,
	).Setup(
		setupHeaderNilWeightHTTPRouteEnv,
	).Assess(
		"Testing header nil weight HTTPRoute feature",
		testHeaderNilWeightHTTPRoute,
	).Teardown(
		teardownHeaderNilWeightHTTPRouteEnv,
	).Feature()
	_ = global.Test(t, feature)
}

func setupHeaderNilWeightHTTPRouteEnv(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	...
	firstHTTPRouteFile, err := os.Open(HTTP_ROUTE_HEADER_NIL_WEIGHT_PATH)
	if err != nil {
		logrus.Errorf("file %q openning was failed: %s", HTTP_ROUTE_HEADER_NIL_WEIGHT_PATH, err)
		t.Error()
		return ctx
	}
	defer firstHTTPRouteFile.Close()
	logrus.Infof("file %q was opened", HTTP_ROUTE_HEADER_NIL_WEIGHT_PATH)
	rolloutFile, err := os.Open(HTTP_ROUTE_HEADER_NIL_WEIGHT_ROLLOUT_PATH)
	if err != nil {
		logrus.Errorf("file %q openning was failed: %s", HTTP_ROUTE_HEADER_NIL_WEIGHT_ROLLOUT_PATH, err)
		t.Error()
		return ctx
	}
	defer rolloutFile.Close()
	logrus.Infof("file %q was opened", HTTP_ROUTE_HEADER_NIL_WEIGHT_ROLLOUT_PATH)
	err = decoder.Decode(firstHTTPRouteFile, &httpRoute)
	if err != nil {
		logrus.Errorf("file %q decoding was failed: %s", HTTP_ROUTE_HEADER_NIL_WEIGHT_PATH, err)
		t.Error()
		return ctx
	}
	logrus.Infof("file %q was decoded", HTTP_ROUTE_HEADER_NIL_WEIGHT_PATH)
	err = decoder.Decode(rolloutFile, &rollout)
	if err != nil {
		logrus.Errorf("file %q decoding was failed: %s", HTTP_ROUTE_HEADER_NIL_WEIGHT_ROLLOUT_PATH, err)
		t.Error()
		return ctx
	}
	logrus.Infof("file %q was decoded", HTTP_ROUTE_HEADER_NIL_WEIGHT_ROLLOUT_PATH)
...
func testHeaderNilWeightHTTPRoute(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	clusterResources := config.Client().Resources()
	...
	logrus.Infof("httpRoute %q was updated with nil weight for header route", resourcesMap[HTTP_ROUTE_KEY].GetName())
...
func teardownHeaderNilWeightHTTPRouteEnv(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	clusterResources := config.Client().Resources()
	resourcesMap, ok := ctx.Value(RESOURCES_MAP_KEY).(map[string]*unstructured.Unstructured)
	if !ok {
		logrus.Errorf("%q type assertion was failed", RESOURCES_MAP_KEY)
...
// getMatchHeaderNilWeightHTTPRouteFetcher returns a fetcher that checks:
...
func getMatchHeaderNilWeightHTTPRouteFetcher(t *testing.T, targetWeight int32, targetHeaderBasedRouteValue gatewayv1.HTTPHeaderMatch) func(k8s.Object) bool {
	return func(obj k8s.Object) bool {
		...
		if !isHeaderBasedHTTPRouteValuesEqual(headerBasedRouteValue, targetHeaderBasedRouteValue) {
			return false
		}
