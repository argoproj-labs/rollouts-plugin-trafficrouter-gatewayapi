...
func setupSingleHeaderBasedHTTPRouteEnv(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
	...
	rolloutFile, err := os.Open(SINGLE_HEADER_BASED_ROLLOUT_PATH)
	if err != nil {
		logrus.Errorf("file %q openning was failed: %s", SINGLE_HEADER_BASED_ROLLOUT_PATH, err)
		t.Error()
		return ctx
	}
	defer rolloutFile.Close()
	logrus.Infof("file %q was opened", SINGLE_HEADER_BASED_ROLLOUT_PATH)
	...
	if err != nil {
		logrus.Errorf("file %q decoding was failed: %s", SINGLE_HEADER_BASED_ROLLOUT_PATH, err)
		t.Error()
		return ctx
	}
	logrus.Infof("file %q was decoded", SINGLE_HEADER_BASED_ROLLOUT_PATH)
